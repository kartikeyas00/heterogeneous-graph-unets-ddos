...
# ============================
# For binary classification, use appropriate torchmetrics classes
...
def train(model, epoch, optimizer, train_dataloader):
    criterion = nn.BCEWithLogitsLoss()
    model.train()
    epoch_loss = 0
    start_time = time.time()
    
    for batch_flow_nodes, subgraph in train_dataloader:
    # Since training is on the full graph, we'll perform it in one step
    ...
    logging.info(f'Epoch {epoch}, Loss: {epoch_loss:.4f}, Time: {epoch_time:.2f}s')
...
def evaluate(model, epoch, test_dataloader):
    ...
    with torch.no_grad():
        for batch_flow_nodes, subgraph in test_dataloader:
            subgraph = subgraph.to(device)
    ...
    tn, fp, fn, tp = confmat.flatten().tolist() #fp is benign misclassification and fn is ddos misclassification

    logging.info(f'Evaluation - Loss: {eval_loss:.4f}, Accuracy: {acc:.4f}, Precision: {prec:.4f}, Recall: {rec:.4f}, F1: {f1:.4f}')
...
def main():
    ...
    for i in range(1,k_folds+1):
        ...
        test_flow_nodes = torch.arange(g.number_of_nodes('flow'))[g.nodes['flow'].data['test_mask']]
        

        train_batches = [train_flow_nodes[i:i + hyperparams['flow_batch_size']] for i in range(0, len(train_flow_nodes), hyperparams['flow_batch_size'])]
        ...
        epochs_no_improve = 0
        logging.info(f'K Fold ----> {i}')
        for epoch in range(1, args.epochs + 1):
            epoch_start_time = time.time()
            
            # Training
            train_loss = train(model,epoch, optimizer, train_dataloader)
            # Evaluation every 5 epochs
            if epoch % 5 == 0:
                acc, prec, rec, f1, eval_loss, tp, fp, fn, tn = evaluate(model, epoch, test_dataloader)
        ...
        if best_f1 > 0:
            ...
            final_acc, final_prec, final_rec, final_f1, final_eval_loss, final_tp, final_fp, final_fn, final_tn = evaluate(model, epoch, test_dataloader)
