...
def parse_args():
    ...
    p.add_argument("--study-name",        type=str, default="egraph_sage_hyperopt")
    return p.parse_args()
...
def create_validation_dataloader(g, config):
    # Get edges with validation mask
...
def objective(trial, loaded_graph):
    ...
    dataloader_key = (config['edge_batch_size'], config['batch_size_train'])
    ...
    val_dataloader_key = (config['edge_batch_size'], config['batch_size_test'])
