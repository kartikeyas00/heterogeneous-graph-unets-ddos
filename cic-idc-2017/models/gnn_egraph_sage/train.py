...
def train(model, epoch, optimizer, train_dataloader):
    ...
    for edge_batches, sub_graph in train_dataloader:
...
def evaluate(model, epoch, test_dataloader):
    ...
    with torch.no_grad():
        for edge_batches, sub_graph in test_dataloader:
            sub_graph = sub_graph.to(device)
...
def main():
    ...
    for i in range(1, k_folds + 1):
        ...
        train_batches = [train_edges_eid[i:i + hyperparams['edge_batch_size']] for i in range(0, len(train_edges_eid), hyperparams['edge_batch_size'])]
        train_batches = [batch for batch in train_batches if len(batch) == hyperparams['edge_batch_size']]
        test_batches = [test_edges_eid[i:i + hyperparams['edge_batch_size']] for i in range(0, len(test_edges_eid), hyperparams['edge_batch_size'])]
        test_batches = [batch for batch in test_batches if len(batch) == hyperparams['edge_batch_size']]
