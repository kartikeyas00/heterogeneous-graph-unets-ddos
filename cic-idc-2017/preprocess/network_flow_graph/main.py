...
from utils import (
    ...
    calculate_feature_stats, build_fold_graph_with_ip_port_nodes, 
    create_kfold_splits
)


def parse_args():
    p = argparse.ArgumentParser(description="Build K-fold graph datasets for network flow analysis with IP-port nodes")
...
def main():
    ...
    for fold_idx, fold in enumerate(fold_data):
        ...
        graph = build_fold_graph_with_ip_port_nodes(
            balanced_train,
            normalized_val,
            normalized_test,
            feature_stats
