...
def create_train_dataloader(g, config):
    ...
    return create_dataloader(g, train_batches, config['batch_size_train'], True, max_connected_flows=config['max_connected_flows'])
...
def create_validation_dataloader(g, config):
    ...
    return create_dataloader(g, val_batches, config['batch_size_validation'], False, max_connected_flows=config['max_connected_flows'])
...
def objective(trial, loaded_graph, loaded_class_weights, in_feats_dict, out_feats_dict, rel_names, canonical_etypes):
    config = {
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        with torch.no_grad():
            ...
            #logger.info(f"Epoch {epoch} - Val Loss: {avg_val_loss:.4f} - Metrics: {results}")
...
def main():
    ...
    study.optimize(lambda t: objective(
                       t, loaded_graph, loaded_class_weights, in_feats_dict, out_feats_dict, rel_names, canonical_etypes
