...
def train_epoch(epoch, train_dataloader, model, optimizer, criterion, device, logger):
    ...
    for edge_eids, subgraph in train_dataloader:
        subgraph = subgraph.to(device)
        edge_features = subgraph.edata['feat'].to(device)
...
def evaluate(epoch, test_dataloader, model, criterion, accuracy_metric, precision_metric,
    ...
    with torch.no_grad():
        for edge_eids, subgraph in test_dataloader:
            subgraph = subgraph.to(device)
            edge_features = subgraph.edata['feat'].to(device)
            node_features = subgraph.ndata['feat'].to(device)
            labels = subgraph.edata['label'].float().to(device)
...
def main():
    ...
    for i in range(1, args.k_folds + 1):
        ...
        test_batches = [test_edges_eid[j:j + hyperparams['edge_batch_size']] 
                       for j in range(0, len(test_edges_eid), hyperparams['edge_batch_size'])]
